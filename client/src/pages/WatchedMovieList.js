...
import YourProfileModal from "../components/YourProfileModal";
        ...
        <>
            <div className="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-6 m-0 p-0 scroll-smooth">
                {props.movies.map((movie) => (
                    ...
                    <p className="mt-1 text-md font-medium text-gray-900"> Rating: {movie.userRating} </p>
                    
                    </div>
                ))}
            </div>
            <YourProfileModal open = {isOpen} onClose={() => setIsOpen(false)}>{plot} </YourProfileModal>
